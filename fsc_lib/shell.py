...
from fsc_lib.cli import client_config as cloud_config
from fsc_lib import clientmanager
from fsc_lib.command import timing
from fsc_lib import exceptions as exc
from fsc_lib.i18n import _
from fsc_lib import logs
from fsc_lib import utils
from fsc_lib import version
...
class FiboStackShell(app.App):
    ...
    def __init__(
        ...
        if not command_manager:
            cm = commandmanager.CommandManager('fibostack.cli')
        else:
            cm = command_manager
    ...
    def run(self, argv):
        ...
        try:
            ret_val = super(FiboStackShell, self).run(argv)
            return ret_val
    ...
    def run_subcommand(self, argv):
        self.init_profile()
        try:
            ret_value = super(FiboStackShell, self).run_subcommand(argv)
        finally:
            self.close_profile()
        return ret_value
    ...
    def interact(self):
        self.init_profile()
        try:
            ret_value = super(FiboStackShell, self).interact()
        finally:
            self.close_profile()
        return ret_value
    ...
    * ClientManager
    ...
    def prepare_to_run_command(self, cmd):
        ...
        self.cloud = self.cloud_config.get_one(
            ...
            additional_user_agent=[('fsc-lib', version.version_string)],
        )
