...
"""FiboStackConfig subclass for argument compatibility"""

import logging

from fibostack.config import exceptions as sdk_exceptions
from fibostack.config import loader as config
...
# Sublcass FiboStackConfig in order to munge config values
# before auth plugins are loaded
class OSC_Config(config.FiboStackConfig):
    ...
    def auth_config_hook(self, config):
        ...
        fibostackclient will override this to perform additional checks
        on auth_type.
        """
    ...
    def _validate_auth(self, config, loader, fixed_argparse=None):
        ...
        if msgs:
            raise sdk_exceptions.FiboStackConfigException('\n'.join(msgs))
        else:
