...
from fibostack.config import cloud_region
from fibostack.config import defaults
...
def make_shell(shell_class=None):
    ...
    if shell_class is None:
        shell_class = shell.FiboStackShell
    _shell = shell_class()
...
class TestShell(TestCase):
    ...
    shell_class_name = "fsc_lib.shell.FiboStackShell"
    ...
    def _assert_initialize_app_arg(self, cmd_options, default_args):
        ...
        fsc_lib.FiboStackShell build_option_parser().  Any global
    ...
    def _assert_cloud_region_arg(self, cmd_options, default_args):
        """Check the args passed to FiboStackConfig.get_one()
        ...
        with mock.patch(
            "fibostack.config.loader.FiboStackConfig.get_one",
            self.occ_get_one,
        ):
            _shell = make_shell(shell_class=self.shell_class)
            _cmd = cmd_options + " module list"
    ...
    def _test_options_get_one_cloud(self, test_opts):
        """Test options sent "to fibostack.config"""
        for opt in test_opts.keys():
            if not test_opts[opt][1]:
                continue
            key = opt2attr(opt)
    ...
    def _test_env_get_one_cloud(self, test_opts):
        """Test environment options sent "to fibostack.config"""
        for opt in test_opts.keys():
            if not test_opts[opt][2]:
                continue
            key = opt2attr(opt)
