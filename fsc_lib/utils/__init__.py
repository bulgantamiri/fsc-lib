...
def get_fsc_show_columns_for_sdk_resource(
    sdk_resource, fsc_column_map, invisible_columns=None
):
    ...
    :param fsc_column_map: A hash of mappings for display column names
    ...
    for sdk_attr, fsc_attr in fsc_column_map.items():
        if sdk_attr in display_columns:
            attr_map[fsc_attr] = sdk_attr
            display_columns.remove(sdk_attr)
        if fsc_attr not in display_columns:
            display_columns.append(fsc_attr)
